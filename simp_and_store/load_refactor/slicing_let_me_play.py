...
#TODO: make the return type becomes: {S2CellID: dict()} instead of {long: dict()}

# This are the directions when chaning the faces. E.g if a feature is on face 0,
# when it cross to the left face of face 0, it is face 4 now. That is recoreded as toLeft {0:4}
...
def slice_feature(json, level):
    ...
    try:
        if geo_type == 'MultiPoint':
            feature_set = slice_multipoint(json, level)
    ...
    # my editor says 'e' is invalid variable name, changed to 'err'... bear with me
    except IndexError, err:
        print ">>> IndexError %s" % err
        print traceback.format_exc()
    except NotImplementedError, err:
        print ">>> NotImplementedError: %s" % err
    return feature_set


def slice_point(point_json, level):
    '''Slice a point feature, given the level. As points are not sliceable,
    it is just stored to corresponding cell'''
    # didn't see a necesarity to deepcopy a point, since we do not modify it.
    # new_point = deepcopy(point_json)
    coord = point_json['geometry']['coordinates']
    cellid = geohelper.get_parent(coord, level)
    return {cellid.id(): point_json}


def slice_multipoint(multipoint_json, level):
    '''Slice a multi-point feature, given the level. The points maybe store to
    different cells depending on their distance and the specified level'''
    # deep copy avoid pollution to the original feature
    new_multipoint = deepcopy(multipoint_json)
    coords = new_multipoint.pop('geometry')['coordinates']
    # after pop, key geomoetry is gone. Then shallow copy could actually work.
    # Question, how faster shallow cp vs deep cp?

    result = {}
    for coord in coords:
        cellid = geohelper.get_parent(coord, level).id()
        if not cellid in result:
            result[cellid] = dict(new_multipoint)   # shallow cp
            result[cellid]['geometry'] = {'coordinates':[], 'type': 'MultiPoint'}
        result[cellid]['geometry']['coordinates'].append(coord)

    # dict().items() returns tuples, which are immutable.
    # in other words, the following for loop does nothing.
    # for cellid, multiPoint in result.items():
    #     if len(multiPoint['geometry']['coordinates']) == 1:
    #         multiPoint['geometry']['coordinates'] = list(
    #             multiPoint['geometry']['coordinates'][0])
    #         multiPoint['geometry']['type'] = 'Point'

    # Thankfully a multipoint with only one coordinate is a valid feature,
    # no need to sanitize anyway.
    # "geometry": { "type": "MultiPoint", "coordinates": [[1,2]] }

    return result


def slice_linestring(linestring_json, level, clockwise=True):
    ''''''
    linestring_json = deepcopy(linestring_json)
    line = linestring_json.pop('geometry')['coordinates']

    pointJson = deepcopy(linestring_json)
    pointJson['geometry']['type'] = 'Point'

    result = {}
    lastCell = lastPoint = None
    for coord in line:
        cell = geohelper.get_parent(coord, level)
        point = S2CellId.FromLatLng(S2LatLng)
        point = s2sphere.CellId.from_lat_lng(
        ...
        while not success:
            ...
            if cell.id() != lastCell.id():
                ...
                if lastCell.face() != cell.face():
                    ...
                    s4, t4 = get_st_for_face(
                        lastPoint.face(), get_closest_xyz(lastPoint, point))
                    if DEBUG_PRINT:
                        print "(Different Face)", get_closest_xyz(lastPoint, point)
                    raise NotImplementedError(
                        "LineStrings on different faces not implemented.")
                ...
                cs1, ct1 = get_st_for_face(lastCell.face(), get_xyz(
                    s2sphere.CellId.from_point(s2sphere.Cell(lastCell).get_vertex(0))))
                cs2, ct2 = get_st_for_face(lastCell.face(), get_xyz(
                    s2sphere.CellId.from_point(s2sphere.Cell(lastCell).get_vertex(2))))
                rightT, leftT, upT, downT = max(
                    cs1, cs2) - s3, min(cs1, cs2) - s3, max(ct1, ct2) - t3, min(ct1, ct2) - t3
                ...
                face, i, j = lastCell.face(), s2sphere.CellId.st_to_ij((decision * (s4 - s3)) +
                                                                       s3), s2sphere.CellId.st_to_ij((decision * (t4 - t3)) + t3)
                fakeS, fakeT = get_st_for_face(nextCell.face(), get_xyz(
                    s2sphere.CellId.from_face_ij(face, i, j)))
                fakePoint = s2sphere.CellId.from_face_ij(
                    nextCell.face(), s2sphere.CellId.st_to_ij(fakeS), s2sphere.CellId.st_to_ij(fakeT))
                if DEBUG_PRINT:
                    if decision == rightT:
                        print "Right!\t",
                    elif decision == leftT:
                ...
                result[lastCell.id()]['geometry']['coordinates'].append([fakePoint.to_lat_lng(
                ).lng().degrees, fakePoint.to_lat_lng().lat().degrees, 'EL' + decisionCode[0]])
                if not nextCell.id() in result:
                    result[nextCell.id()] = deepcopy(lineStringJson)
                result[nextCell.id()]['geometry']['coordinates'].append([fakePoint.to_lat_lng(
                ).lng().degrees, fakePoint.to_lat_lng().lat().degrees, 'EE' + decisionCode[1]])

                # Wrap the Corner (if necessary)
                if len(result[nextCell.id()]['geometry']['coordinates']) > 1:
                    newPoints = wrapCorner(nextCell.id(), clockwise, result[nextCell.id(
                    )]['geometry']['coordinates'][-2][2], result[nextCell.id()]['geometry']['coordinates'][-1][2])
                    if newPoints != None:
                        result[nextCell.id()
                               ]['geometry']['coordinates'][-1:-1] = newPoints

                lastCell, lastPoint = nextCell, fakePoint

            else:
                result[cell.id()]['geometry']['coordinates'].append(
                    [point.to_lat_lng().lng().degrees, point.to_lat_lng().lat().degrees])
                lastPoint = point
                success = True
    ...
    for line in lines:
        ...
        for location in lineLocations.keys():
            ...
            result[location]['geometry']['coordinates'].append(
                lineLocations[location]['geometry']['coordinates'])
...
def slicePolygon(polygonJsonParam, level):
    ...
    if len(lines) > 1:
        raise NotImplementedError(
            "Polygons with holes are not yet implemented.")
    ...
    for line in lines:
        ...
        for location in lineLocations.keys():
            ...
            result[location]['geometry']['coordinates'][0].extend(
                lineLocations[location]['geometry']['coordinates'])
    ...
    for location in result:
        for pointNum, point in enumerate(result[location]['geometry']['coordinates'][0]):
            if len(point) == 3:
                ...
                if point[2][1] == 'E' and pointNum != 0:
                    newPoints = wrapCorner(location, clockwise, result[location]['geometry']['coordinates']
                                           [0][pointNum - 1][2], result[location]['geometry']['coordinates'][0][pointNum][2])
                    if newPoints != None:
        ...
        if result[location]['geometry']['coordinates'][0][0] != result[location]['geometry']['coordinates'][0][-1]:
            newPoints = wrapCorner(location, clockwise, result[location]['geometry']['coordinates']
                                   [0][-1][2], result[location]['geometry']['coordinates'][0][0][2])
            if newPoints != None:
                result[location]['geometry']['coordinates'][0].extend(
                    newPoints)
            result[location]['geometry']['coordinates'][0].append(
                result[location]['geometry']['coordinates'][0][0])
...
def sliceMultiPolygon(multiPolygonJson, level):
    ...
    for polygon in polygons:
        ...
        for location in polygonLocations.keys():
            ...
            result[location]['geometry']['coordinates'].append(
                polygonLocations[location]['geometry']['coordinates'])
...
def wrapCorner(s2_id, clockwise, start_code, end_code):
    ...
    while start != end:
        ...
        result.append(
            [latlng.lng().degrees, latlng.lat().degrees, 'C' + curEdge + nextEdge])
...
def get_closest_xyz(srcCell, destCell):
    ...
    # Adjacent Faces
        ...
        if toLeft[srcFace] == destFace:
            ...
            xb, yb, zb = translate(offset[0] + rot[1][0] - rot[0][0], offset[1] +
                                   rot[1][1] - rot[0][1], offset[2] + rot[1][2] - rot[0][2], xb, yb, zb)
            ...
            x, y, z = translate(-offset[0] - rot[0][0], -offset[1] -
                                rot[0][1], -offset[2] - rot[0][2], x, y, z)
            x, y, z = rotate(-rot[1][0], -rot[1][1], -rot[1][2], x, y, z)
            x, y, z = translate(
                offset[0] + rot[0][0], offset[1] + rot[0][1], offset[2] + rot[0][2], x, y, z)
            score1 = distance(srcX, srcY, srcZ, x, y, z)

            xa, ya, za = translate(-offset[0] - rot[0][0], -
                                   offset[1] - rot[0][1], -offset[2] - rot[0][2], x, y, z)
            xa, ya, za = rotate(-rot[2][0], -rot[2][1], -rot[2][2], xa, ya, za)
            xa, ya, za = translate(
                offset[0] + rot[0][0], offset[1] + rot[0][1], offset[2] + rot[0][2], xa, ya, za)
            score2 = distance(srcX, srcY, srcZ, xa, ya, za)

            xb, yb, zb = translate(-offset[0] - rot[0][0], -
                                   offset[1] - rot[0][1], -offset[2] - rot[0][2], x, y, z)
            xb, yb, zb = rotate(rot[2][0], rot[2][1], rot[2][2], xb, yb, zb)
            xb, yb, zb = translate(offset[0] + rot[0][0] + rot[1][0], offset[1] +
                                   rot[0][1] + rot[1][1], offset[2] + rot[0][2] + rot[1][2], xb, yb, zb)
            score3 = distance(srcX, srcY, srcZ, xb, yb, zb)
            ...
            x, y, z = translate(-offset[0] - rot[1][0], -offset[1] -
                                rot[1][1], -offset[2] - rot[1][2], x, y, z)
            x, y, z = rotate(rot[0][0], rot[0][1], rot[0][2], x, y, z)
            x, y, z = translate(
                offset[0] + rot[1][0], offset[1] + rot[1][1], offset[2] + rot[1][2], x, y, z)
